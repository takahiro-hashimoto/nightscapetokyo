// Supabase REST (PostgREST) 経由の全テーブルJSONバックアップ。
// OpenAPI スキーマからテーブル一覧を取得し、1000行ずつページングで全件保存する。
...
const dir = path.join('backups', `nightscape-${stamp}`);
fs.mkdirSync(dir, { recursive: true });

// テーブル一覧を OpenAPI から取得
...
for (const t of tables) {
  ...
  console.log(`  ${t.padEnd(28)} ${String(rows.length).padStart(6)} 行  ${(body.length/1024).toFixed(0).padStart(6)} KB`);
}
