...
$("#datepicker").datepicker({
    ...
    defaultDate: new Date(), // 初期表示の日付を設定
...
function drawSunPaths(map, marker, selectedDate) {
    ...
    if (result.sunriseAzimuth !== null) {
        sunriseLine = drawSunPath(map, [position.lat, position.lng], result.sunriseAzimuth, '#F86C5C');
    }
    if (result.sunsetAzimuth !== null) {
        sunsetLine = drawSunPath(map, [position.lat, position.lng], result.sunsetAzimuth, '#F86C5C');
    }
}
