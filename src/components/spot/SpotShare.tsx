...
const LOCALE_BUTTONS: Record<string, readonly string[]> = {
  ...
  ko: KAKAO_APP_KEY ? ["x", "kakao", "line", "copy"] : ["x", "line", "copy"],
...
function KakaoIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M12 3C6.477 3 2 6.477 2 10.857c0 2.796 1.62 5.254 4.073 6.756l-1.04 3.872a.25.25 0 0 0 .372.277L9.77 19.44A11.39 11.39 0 0 0 12 19.714c5.523 0 10-3.477 10-7.857S17.523 3 12 3z" />
    </svg>
  );
}
  ...
  return (
    <aside className={`share-section${className ? ` ${className}` : ""}`} aria-labelledby="share-heading">
      ...
      <div className="share-buttons">
        ...
        {buttons.includes("kakao") && (
          <button type="button" className="share-btn share-btn-kakao" onClick={shareOnKakao}>
            <KakaoIcon />
            <span>{labels.kakao ?? "카카오톡"}</span>
          </button>
        )}
