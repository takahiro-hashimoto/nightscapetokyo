...
import { getTagBySlug, getSpotsByTagSlug, getSpotsBySlugs, getTagPageBySlug, getAvailableTagPageLocales } from "@/lib/supabase/queries";
...
/** 同一リクエスト内で getTagPageBySlug の重複呼び出しを排除 */
const getCachedTagPage = cache((slug: string) => getTagPageBySlug(slug));
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const dbPage = await getCachedTagPage(slug);
...
export default async function TagPage({ params }: Props) {
  ...
  const [tag, spotsByTag] = await Promise.all([
    getTagBySlug(slug),
    getSpotsByTagSlug(slug),
  ]);
