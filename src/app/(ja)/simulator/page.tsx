...
export const metadata: Metadata = {
  ...
  description:
    "日の出・日の入りの方角（方位角）と時刻を地図上で無料で確認できる計算ツールです。場所・日付を指定するだけで朝日・夕日の方向を即表示。初日の出の下見・ダイヤモンド富士・夕日撮影の構図決め・マンションの日当たり確認にご活用ください。",
  keywords: [
    "日の出 方角",
    "日の入り 方角",
    "日の出 方角 調べ方",
...
const FAQ = [
  {
    q: "日の出の方角の調べ方は？",
    a: "このツールで地図上のマーカーを調べたい地点に移動するだけで、日の出の方角（方位角）が自動計算されます。日付を指定すると夏至・冬至など特定の日の方角も確認できます。",
  },
  {
    q: "日の出・日の入り時刻を無料で計算するには？",
    a: "このWebアプリは完全無料でご利用いただけます。場所（地名・住所・現在地）と日付を指定するだけで、その地点の日の出・日の入り時刻が自動で計算・表示されます。",
  },
  {
    q: "ダイヤモンド富士の方角を調べるには？",
    a: "富士山が見える撮影スポットにマーカーを置き、ダイヤモンド富士が起きやすい時期（春分・秋分の前後）の日付を指定すると、日の入り方角が富士山頂と一致するかを事前に確認できます。",
  },
  {
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify([
            ...
            },
            {
              ...
              keywords:
                "日の出 方角, 日の入り 方角, 日の出 方角 調べ方, 日の出 時刻 計算, 日の出 シミュレーター, 日の出 方角 シミュレーター 無料, 朝日 方角, 夕日 方角, 初日の出 方角, ダイヤモンド富士 方角",
