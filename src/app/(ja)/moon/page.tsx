...
const OG_TITLE =
  "月の出・月の入り方角シミュレーター【無料】｜月出方位角・月齢を地図で確認";
const OG_DESC =
...
export const metadata: Metadata = {
  ...
  description:
    "月の出・月の入りの方角（月出方位角）と時刻を地図上で無料で確認できる計算ツールです。場所・日付を指定するだけで月の出る方向を即表示。今夜の月はどこから昇るか、次の満月はいつか、月齢カレンダーで一覧確認できます。",
  keywords: [
    "月の出 方角",
    "月の入り 方角",
    "月の出 方角 調べ方",
...
const STATIC_FAQ = [
  ...
  {
    q: "今夜の月はどこから昇る？",
    a: "今日の日付と自分の現在地（または調べたい地点）を設定すると、今夜の月の出方角と時刻が地図上の方位線で表示されます。現在地ボタンを使えば位置設定も簡単です。",
  },
  {
    q: "月の出時刻を無料で計算するには？",
    a: "このWebアプリは完全無料でご利用いただけます。場所（地名・住所・現在地）と日付を指定するだけで、その地点の月の出・月の入り時刻が自動で計算・表示されます。",
  },
  {
    q: "次の満月はいつですか？",
    a: "月齢カレンダーに各日の月齢が表示されます。月齢15前後の日が満月にあたります。カレンダーを翌月に進めることで次の満月の日程も事前に確認できます。",
  },
  {
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify([
            ...
            },
            {
              ...
              keywords:
                "月の出 方角, 月の入り 方角, 月の出 方角 調べ方, 月の出 時刻 計算, 月の出 シミュレーター, 月の出 方角 シミュレーター 無料, 今夜の月 方角, 満月 方角, 次の満月 いつ, 月出方位角",
